...
internal class UniqueItemsKeyword : KeywordBase
{
    ...
    protected internal override ValidationResult ValidateCore(JsonInstanceElement instance, JsonSchemaOptions options)
    {
        ...
        for (int i = 0; i < items.Length; i++)
        {
            ...
            {
                if (items[j].Equivalent(curItem, options.JsonArrayEqualityComparer).Result)
                {
                    return ValidationResult.SingleErrorFailedResult(new ValidationError(ErrorMessage(curItem.ToString(), i, j), options.ValidationPathStack, Name, instance.Location));
                }
            }
        }
